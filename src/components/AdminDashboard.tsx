  ...
  return (
    <div className="space-y-8 pb-12">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6">
        <div>
          ...
          <p className="text-slate-500 font-mono text-xs uppercase tracking-widest">
            {isMainAdmin ? 'Global Festival Oversight' : 'Event Specific Management'}
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        <StatCard title="TOTAL REGISTRATIONS" value={stats.totalRegs.toString()} change="+0%" icon={<Users />} color="indigo" />
        <StatCard title="ACTIVE EVENTS" value={stats.activeEvents.toString()} change="0" icon={<Calendar />} color="blue" />
        <StatCard title="REVENUE (TOKEN)" value={stats.revenue.toLocaleString()} change="+0%" icon={<DollarSign />} color="green" />
        <StatCard title="TEAM FORMATION" value={`${stats.teamFormation}%`} change="+0%" icon={<TrendingUp />} color="purple" />
      </div>
